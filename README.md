...
## Running Validator Node (Testnet)
...
(validator)$ x1 account new
```

After entering the command, you will get prompted to enter a password for the account (= wallet) — use a strong one!
You can e.g. use a password manager to generate a 20+ digit password to secure your wallet.

### Fund your validator wallet
The next step is to fund your validator wallet with enough XN to become a validator. 
That means you need to have at least 500,000 XN in the wallet you just created (send a little more to cover transaction fees).
...
(validator)$ x1 attach
```
Now initialize the SFC contract ABI variable [sfc.abi.json](sfc.abi.json):
...
```shell
(validator)$ x1 --testnet \
        --validator.id ID --validator.pubkey 0xPubkey --validator.password /path/to/password_file
```
